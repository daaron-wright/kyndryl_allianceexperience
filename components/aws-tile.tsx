...
import AWSVideoModal from "./aws-video-modal"

interface AWSTileProps {
  onClick?: () => void
}
...
export default function AWSTile({ onClick }: AWSTileProps) {
  ...
  return (
    <>
      ...
      >
        ...
        <div className="absolute bottom-0 left-0 w-full h-1/2 flex flex-col">
          ...
          <div style={{ padding: "clamp(20px, 3vw, 40px) clamp(20px, 3vw, 40px) clamp(10px, 1.5vw, 20px)" }}>
            ...
            >
              Scale Innovation with AWS Cloud
            </h2>
          </div>
          ...
          >
            ...
            >
              Kyndryl's infrastructure expertise combines with AWS's comprehensive cloud services to deliver scalable,
              secure solutions that accelerate your digital journey
            </p>
          </div>
        </div>
      ...
      {isVideoModalOpen && <AWSVideoModal onClose={() => setIsVideoModalOpen(false)} />}
    </>
  )
}
